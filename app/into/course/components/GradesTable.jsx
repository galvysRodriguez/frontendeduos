...
// PDF Export Function for the grades summary
const handleExportGradesSummary = (data) => {
    const doc = new jsPDF();
    doc.setFontSize(16);
...
const handleExportLapsoDetails = (lapso, subject, lapsoIndex) => {
    ...
    doc.save(`detalles_lapso_${lapsoIndex + 1}_${subject}.pdf`);
    toast.success("Detalles del lapso exportados.");
};

// New PDF Export Function for the full academic history
const handleExportFullHistory = (data) => {
    const doc = new jsPDF();
    let currentY = 20;

    doc.setFontSize(16);
    doc.setTextColor("#003366");
    doc.text("Historial Académico Completo", 14, currentY);
    currentY += 10;

    // Table of contents-like summary table
    autoTable(doc, {
        startY: currentY,
        head: [['Asignatura', 'Lapso 1', 'Lapso 2', 'Lapso 3', 'Definitiva']],
        body: data.map(row => [
    ...
    // Detailed section for each subject and lapso
    data.forEach(subjectData => {
        doc.addPage();
        currentY = 20;
        doc.setFontSize(14);
        doc.setTextColor("#0077B6");
        doc.text(`Asignatura: ${subjectData.subject}`, 14, currentY);
        currentY += 10;

        subjectData.lapsos.forEach((lapso, index) => {
            doc.setFontSize(12);
            doc.setTextColor("#003366");
            doc.text(`Lapso ${index + 1}: Nota Final: ${lapso.finalGrade !== null ? lapso.finalGrade : 'Pendiente'}`, 14, currentY);
            currentY += 5;
    ...
    doc.save("historial_academico_completo.pdf");
    toast.success("Historial académico completo exportado.");
};
        ...
        <div className="container mx-auto py-10 bg-gradient-to-br from-[#F0F8FF] to-[#E0F2F7] min-h-screen">
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg max-w-5xl mx-auto border border-gray-100">
                ...
                <div className="flex justify-end space-x-4 mb-4">
                    <Button onClick={() => handleExportGradesSummary(gradesData)} className="bg-[#003366] hover:bg-[#002244] text-white">Exportar Resumen a PDF</Button>
                </div>
